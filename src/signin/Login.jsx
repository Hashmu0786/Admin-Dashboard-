...
// import { signin } from "../reduxToolkit/loginSlice";
...
//   const addStatus = useSelector((state)=> state.login.addStatus)
...
import { AiOutlineEye, AiFillEyeInvisible } from "react-icons/ai";
import { useNavigate } from "react-router-dom";
import { signin, addTokenToLocalStorage } from "../reduxToolkit/loginSlice";
import { useDispatch, useSelector } from "react-redux";
    ...
    if (Object.keys(formErrors).length === 0) {
      dispatch(signin({ email, password })).then((action) => {
        if (signin.fulfilled.match(action)) {
          dispatch(addTokenToLocalStorage(action.payload));
          navigate("/layout");
        }
      });
    }
